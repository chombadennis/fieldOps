...
class TechDocument(Base, CustomBase):
    ...
    items = relationship("TechItem", back_populates="document", cascade="all, delete-orphan")

class TechItem(Base, CustomBase):
    __tablename__ = 'tech_items'
    
    id = Column(Integer, primary_key=True, index=True)
    document_id = Column(Integer, ForeignKey('tech_documents.id', ondelete="CASCADE"), nullable=False)
    
    description = Column(Text, nullable=False)
    values_map = Column(JSONB, nullable=True, default=None)
    
    document = relationship("TechDocument", back_populates="items")

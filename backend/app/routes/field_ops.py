...
router = APIRouter(
    prefix="/projects/{project_id}/field_ops",
    tags=["Field Ops Documents"]
)

@router.get("", response_model=List[platform_schemas.Document])
def get_field_ops_documents(
    project_id: int,
    ...
    query = db.query(FieldOpsDocument)
    if hasattr(FieldOpsDocument, 'project_id'):
        query = query.filter(FieldOpsDocument.project_id == project_id)
        
    if hasattr(FieldOpsDocument, 'contract_id') and contract_id is not None:
        query = query.filter(FieldOpsDocument.contract_id == contract_id)

    docs = query.order_by(FieldOpsDocument.created_at.desc()).all()
    result = []
    for d in docs:
        result.append(
...
def create_field_ops_document(
    ...
    if hasattr(FieldOpsDocument, 'project_id'):
        query = query.filter(FieldOpsDocument.project_id == project_id)
    if hasattr(FieldOpsDocument, 'contract_id'):
        query = query.filter(FieldOpsDocument.contract_id == contract_id)
        
    if doc_in.cloud_file_id and hasattr(FieldOpsDocument, 'cloud_file_id'):
        existing_doc = query.filter(FieldOpsDocument.cloud_file_id == doc_in.cloud_file_id).first()
    
    if not existing_doc and doc_in.file_url:
        existing_doc = query.filter(FieldOpsDocument.file_url == doc_in.file_url).first()
    ...
    if hasattr(FieldOpsDocument, 'cloud_file_id'):
        kwargs["cloud_file_id"] = doc_in.cloud_file_id
    if hasattr(FieldOpsDocument, 'file_size'):
        kwargs["file_size"] = doc_in.file_size
    if hasattr(FieldOpsDocument, 'is_linked'):

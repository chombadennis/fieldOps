...
router = APIRouter(
    prefix="/projects/{project_id}/reimbursable_claims",
    tags=["Reimbursable Claim Documents"]
)

@router.get("", response_model=List[platform_schemas.Document])
def get_reimbursable_claims_documents(
    project_id: int,
    ...
    query = db.query(ReimbursableClaimDocument)
    if hasattr(ReimbursableClaimDocument, 'project_id'):
        query = query.filter(ReimbursableClaimDocument.project_id == project_id)
        
    if hasattr(ReimbursableClaimDocument, 'contract_id') and contract_id is not None:
        query = query.filter(ReimbursableClaimDocument.contract_id == contract_id)

    docs = query.order_by(ReimbursableClaimDocument.created_at.desc()).all()
    result = []
    for d in docs:
        result.append(
            platform_schemas.Document(
                ...
                department="reimbursable_claims",
...
def create_reimbursable_claims_document(
    project_id: int,
    ...
    if hasattr(ReimbursableClaimDocument, 'project_id'):
        query = query.filter(ReimbursableClaimDocument.project_id == project_id)
    if hasattr(ReimbursableClaimDocument, 'contract_id'):
        query = query.filter(ReimbursableClaimDocument.contract_id == contract_id)
        
    if doc_in.cloud_file_id and hasattr(ReimbursableClaimDocument, 'cloud_file_id'):
        existing_doc = query.filter(ReimbursableClaimDocument.cloud_file_id == doc_in.cloud_file_id).first()
    
    if not existing_doc and doc_in.file_url:
        existing_doc = query.filter(ReimbursableClaimDocument.file_url == doc_in.file_url).first()
    ...
    if hasattr(ReimbursableClaimDocument, 'cloud_file_id'):
        kwargs["cloud_file_id"] = doc_in.cloud_file_id
    if hasattr(ReimbursableClaimDocument, 'file_size'):
        kwargs["file_size"] = doc_in.file_size
    if hasattr(ReimbursableClaimDocument, 'is_linked'):
        kwargs["is_linked"] = True
        kwargs["linked_at"] = func.now()

    new_doc = ReimbursableClaimDocument(**kwargs)
    db.add(new_doc)
    db.commit()
    db.refresh(new_doc)
    d = new_doc
...
def unlink_reimbursable_claims_document(
    project_id: int,
    document_id: int,
    db: Session = Depends(get_db)
):
    doc = db.query(ReimbursableClaimDocument).filter(ReimbursableClaimDocument.id == document_id).first()
    if not doc:
...
def delete_reimbursable_claims_document(
    project_id: int,
    document_id: int,
    db: Session = Depends(get_db)
):

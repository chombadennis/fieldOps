...
router = APIRouter(
    prefix="/projects/{project_id}/budget",
    tags=["Budget Documents"]
)
...
def get_budget_documents(
    ...
    query = db.query(BudgetDocument)
    if hasattr(BudgetDocument, 'project_id'):
        query = query.filter(BudgetDocument.project_id == project_id)
        
    if hasattr(BudgetDocument, 'contract_id') and contract_id is not None:
        query = query.filter(BudgetDocument.contract_id == contract_id)

    docs = query.order_by(BudgetDocument.created_at.desc()).all()
    result = []
    for d in docs:
        result.append(
...
def create_budget_document(
    ...
    if hasattr(BudgetDocument, 'project_id'):
        query = query.filter(BudgetDocument.project_id == project_id)
    if hasattr(BudgetDocument, 'contract_id'):
        query = query.filter(BudgetDocument.contract_id == contract_id)
        
    if doc_in.cloud_file_id and hasattr(BudgetDocument, 'cloud_file_id'):
        existing_doc = query.filter(BudgetDocument.cloud_file_id == doc_in.cloud_file_id).first()
    
    if not existing_doc and doc_in.file_url:
        existing_doc = query.filter(BudgetDocument.file_url == doc_in.file_url).first()
    ...
    if not budget:
        budget = Budget(project_id=project_id, name="General Budget", amount=0.0)
        db.add(budget)
        db.commit()
        db.refresh(budget)
    ...
    if hasattr(BudgetDocument, 'cloud_file_id'):
        kwargs["cloud_file_id"] = doc_in.cloud_file_id
    if hasattr(BudgetDocument, 'file_size'):
        kwargs["file_size"] = doc_in.file_size
    if hasattr(BudgetDocument, 'is_linked'):
        kwargs["is_linked"] = True
        kwargs["linked_at"] = func.now()

    new_doc = BudgetDocument(**kwargs)
    db.add(new_doc)
    db.commit()
    db.refresh(new_doc)
    d = new_doc

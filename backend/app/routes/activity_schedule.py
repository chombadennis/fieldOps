...
router = APIRouter(
    prefix="/projects/{project_id}/activity_schedule",
    tags=["Activity Schedule Documents"]
)

@router.get("", response_model=List[platform_schemas.Document])
def get_activity_schedule_documents(
    project_id: int,
    ...
    query = db.query(ActivityScheduleDocument)
    if hasattr(ActivityScheduleDocument, 'project_id'):
        query = query.filter(ActivityScheduleDocument.project_id == project_id)
        
    if hasattr(ActivityScheduleDocument, 'contract_id') and contract_id is not None:
        query = query.filter(ActivityScheduleDocument.contract_id == contract_id)

    docs = query.order_by(ActivityScheduleDocument.created_at.desc()).all()
    result = []
    for d in docs:
        result.append(
...
def create_activity_schedule_document(
    project_id: int,
    ...
    if hasattr(ActivityScheduleDocument, 'project_id'):
        query = query.filter(ActivityScheduleDocument.project_id == project_id)
    if hasattr(ActivityScheduleDocument, 'contract_id'):
        query = query.filter(ActivityScheduleDocument.contract_id == contract_id)
        
    if doc_in.cloud_file_id and hasattr(ActivityScheduleDocument, 'cloud_file_id'):
        existing_doc = query.filter(ActivityScheduleDocument.cloud_file_id == doc_in.cloud_file_id).first()
    
    if not existing_doc and doc_in.file_url:
        existing_doc = query.filter(ActivityScheduleDocument.file_url == doc_in.file_url).first()
    ...
    if hasattr(ActivityScheduleDocument, 'cloud_file_id'):
        kwargs["cloud_file_id"] = doc_in.cloud_file_id
    if hasattr(ActivityScheduleDocument, 'file_size'):
        kwargs["file_size"] = doc_in.file_size
    if hasattr(ActivityScheduleDocument, 'is_linked'):
        kwargs["is_linked"] = True
        kwargs["linked_at"] = func.now()

    new_doc = ActivityScheduleDocument(**kwargs)
    db.add(new_doc)
    db.commit()
    db.refresh(new_doc)
    d = new_doc
...
def unlink_activity_schedule_document(
    project_id: int,
    document_id: int,
    db: Session = Depends(get_db)
):
    doc = db.query(ActivityScheduleDocument).filter(ActivityScheduleDocument.id == document_id).first()
    if not doc:
...
def delete_activity_schedule_document(
    project_id: int,
    document_id: int,
    db: Session = Depends(get_db)
):

...
def get_tech_documents(
    ...
    query = db.query(TechDocument)
    if hasattr(TechDocument, 'project_id'):
        query = query.filter(TechDocument.project_id == project_id)
        
    if hasattr(TechDocument, 'contract_id') and contract_id is not None:
        query = query.filter(TechDocument.contract_id == contract_id)

    docs = query.order_by(TechDocument.created_at.desc()).all()
    result = []
    for d in docs:
        result.append(
...
def create_tech_document(
    ...
    if hasattr(TechDocument, 'project_id'):
        query = query.filter(TechDocument.project_id == project_id)
    if hasattr(TechDocument, 'contract_id'):
        query = query.filter(TechDocument.contract_id == contract_id)
        
    if doc_in.cloud_file_id and hasattr(TechDocument, 'cloud_file_id'):
        existing_doc = query.filter(TechDocument.cloud_file_id == doc_in.cloud_file_id).first()
    
    if not existing_doc and doc_in.file_url:
        existing_doc = query.filter(TechDocument.file_url == doc_in.file_url).first()
    ...
    if hasattr(TechDocument, 'cloud_file_id'):
        kwargs["cloud_file_id"] = doc_in.cloud_file_id
    if hasattr(TechDocument, 'file_size'):
        kwargs["file_size"] = doc_in.file_size
    if hasattr(TechDocument, 'is_linked'):
        kwargs["is_linked"] = True
        kwargs["linked_at"] = func.now()

    new_doc = TechDocument(**kwargs)
    db.add(new_doc)
    db.commit()
    db.refresh(new_doc)
    d = new_doc
...
def unlink_tech_document(
    project_id: int,
    document_id: int,
    db: Session = Depends(get_db)
):
    doc = db.query(TechDocument).filter(TechDocument.id == document_id).first()
    if not doc:

...
router = APIRouter(
    prefix="/projects/{project_id}/program_of_works",
    tags=["Program Of Works Documents"]
)

@router.get("", response_model=List[platform_schemas.Document])
def get_program_of_works_documents(
    project_id: int,
    ...
    query = db.query(ProgramOfWorksDocument)
    if hasattr(ProgramOfWorksDocument, 'project_id'):
        query = query.filter(ProgramOfWorksDocument.project_id == project_id)
        
    if hasattr(ProgramOfWorksDocument, 'contract_id') and contract_id is not None:
        query = query.filter(ProgramOfWorksDocument.contract_id == contract_id)

    docs = query.order_by(ProgramOfWorksDocument.created_at.desc()).all()
...
def create_program_of_works_document(
    project_id: int,
    ...
    if hasattr(ProgramOfWorksDocument, 'project_id'):
        query = query.filter(ProgramOfWorksDocument.project_id == project_id)
    if hasattr(ProgramOfWorksDocument, 'contract_id'):
        query = query.filter(ProgramOfWorksDocument.contract_id == contract_id)
        
    if doc_in.cloud_file_id and hasattr(ProgramOfWorksDocument, 'cloud_file_id'):
        existing_doc = query.filter(ProgramOfWorksDocument.cloud_file_id == doc_in.cloud_file_id).first()
    
    if not existing_doc and doc_in.file_url:
        existing_doc = query.filter(ProgramOfWorksDocument.file_url == doc_in.file_url).first()
    ...
    if hasattr(ProgramOfWorksDocument, 'cloud_file_id'):
        kwargs["cloud_file_id"] = doc_in.cloud_file_id
    if hasattr(ProgramOfWorksDocument, 'file_size'):
        kwargs["file_size"] = doc_in.file_size
    if hasattr(ProgramOfWorksDocument, 'is_linked'):
        kwargs["is_linked"] = True
        kwargs["linked_at"] = func.now()

    new_doc = ProgramOfWorksDocument(**kwargs)
    db.add(new_doc)
    db.commit()
    db.refresh(new_doc)
    d = new_doc
...
def unlink_program_of_works_document(
    project_id: int,
    document_id: int,
    db: Session = Depends(get_db)
):
    doc = db.query(ProgramOfWorksDocument).filter(ProgramOfWorksDocument.id == document_id).first()
    if not doc:
...
def delete_program_of_works_document(
    project_id: int,
    document_id: int,
    db: Session = Depends(get_db)
):

...
import { ArrowLeft, Home, AlertTriangle, X, FileSpreadsheet, DollarSign, FileCheck, HardHat, Wrench, Users, Scale, Building2, Calendar, ClipboardList, Ruler, Truck, Clock, Plus, Settings, Trash2, Activity } from 'lucide-react';
...
export default function ProjectDashboardPage({ params }: { params: { projectId: string } }) {
  ...
  const { data: documents = EMPTY_ARRAY } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [dData, techData, fieldOpsData, activityData, milestoneData, rateData, reimbursableData, programData, ipcDocsData, boqData, budgetDocsData] = await Promise.all([
        ...
        api.getDecoupledDocuments(projectId, 'boq').catch(() => []),
        api.getDecoupledDocuments(projectId, 'budget').catch(() => []),
      ]);
      return [...dData, ...techData, ...fieldOpsData, ...activityData, ...milestoneData, ...rateData, ...reimbursableData, ...programData, ...ipcDocsData, ...boqData, ...budgetDocsData];
    }
  });
  ...
  const [activeTab, setActiveTab] = useState<string>(() => {
    if (typeof window !== 'undefined') {
      return new URLSearchParams(window.location.search).get('active_tab') || 'activity';
    }
    return 'activity';
  });
    ...
    <div className="min-h-screen bg-[#030305] text-white selection:bg-neon-cyan/30 pb-20 relative font-inter overflow-hidden">
      ...
      <main className="container mx-auto p-4 space-y-6 relative z-10">
        <div className="flex items-center justify-between mb-2">
          ...
          {/* Notification Bell — user id will come from auth context */}
        ...
        {/* Top Level Tab 0: Activity Feed */}
        {activeTab === 'activity' && (
          <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-6 shadow-[0_8px_32px_rgba(0,0,0,0.5)] border border-white/10">
            <ActivityFeed projectId={Number(projectId)} />
          </div>
        )}
        ...
        {activeTab === 'pmo' && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className={`w-full flex-shrink-0 space-y-4 transition-all duration-300 ${isLeftSidebarCollapsed ? 'lg:w-20' : 'lg:w-72'}`}>
                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-4 shadow-[0_8px_32px_rgba(0,0,0,0.5)] space-y-3 sticky top-6 max-h-[calc(100vh-3rem)] overflow-y-auto custom-scrollbar">
                  <div className={`px-3 py-2 bg-black/40 rounded-2xl border border-white/5 flex items-center ${isLeftSidebarCollapsed ? 'justify-center' : 'justify-between'}`}>
                    ...
                    <button onClick={() => setIsLeftSidebarCollapsed(!isLeftSidebarCollapsed)} className="p-1.5 hover:bg-white/10 rounded-lg text-gray-400 hover:text-white transition-colors" title="Toggle Sidebar">
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d={isLeftSidebarCollapsed ? "M9 18l6-6-6-6" : "M15 18l-6-6 6-6"} /></svg>
                    </button>
                  </div>
                  ...
                  <div className="space-y-2">
                    {PMO_SUBTABS.map((sub) => {
                      ...
                      return (
                        ...
                        >
                          <div className={`flex items-center ${isLeftSidebarCollapsed ? 'justify-center w-full' : 'space-x-3'}`}>
                            <div
                          ...
                          {sub.badgeCount !== undefined && !isLeftSidebarCollapsed && (
                            <span
                          ...
                          {sub.badgeCount !== undefined && isLeftSidebarCollapsed && sub.badgeCount > 0 && (
                            <div className="absolute top-2 right-2 w-2 h-2 bg-neon-cyan rounded-full shadow-[0_0_8px_rgba(0,243,255,0.8)]" />
                          )}
                        </button>
                      );
                    })}
                ...
                <div key={pmoSubTab} className="space-y-8 animate-fade-in-up">
                  ...
                  {pmoSubTab === 'boq' && (
                    <div className="space-y-8 animate-fade-in">
                      ...
                      {/* Linked BOQ Documents */}
                      <div className="mt-8">
                        <LinkedDocumentsPanel
                          projectId={project.id}
                          documents={documents.filter((d: any) => d.department?.toLowerCase() === 'boq')}
                          title="Attached Bills of Quantities"
                          emptyMessage="No BOQ documents attached yet."
                          onUnlink={async (id: number) => {
                            await deleteDecoupledDocument(projectId, 'boq', id);
                            fetchProjectData();
                          }}
                          onViewBoqItems={handleViewBoqItems}
                        />
                      </div>

                        </div>
                        <div className="shrink-0">
                          {/* Collaboration Panel for BoQ */}
                          {project?.id && (
                            <CollaborationPanel
                              projectId={project.id}
                              module="BoQ"
                              moduleName="Bill of Quantities"
                            />
                          )}
                        </div>
                      </div>
                    ...
                    {FIELD_OPS_SUBTABS.filter(sub => !hiddenModules.includes(sub.key)).map((sub) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {sub.badgeCount !== undefined && sub.badgeCount > 0 && !isLeftSidebarCollapsed && (
                            <span
                              className={`ml-2 px-2 py-0.5 text-[10px] font-black rounded-full whitespace-nowrap ${isSubActive
                          ...
                          {sub.badgeCount !== undefined && isLeftSidebarCollapsed && sub.badgeCount > 0 && (
                            <div className="absolute top-2 right-2 w-2 h-2 bg-neon-pink rounded-full shadow-[0_0_8px_rgba(255,0,127,0.8)]" />
                          )}
                        </button>
                      );
                    })}
        ...
        <div className="mt-12 bg-gradient-to-br from-white/5 to-white/5 border-t border-white/10 pt-8 pb-4 relative z-10 text-center">
          <div className="max-w-4xl mx-auto px-4">
            <div className="flex items-center justify-center mb-4">
              <div className="h-[1px] w-12 bg-gradient-to-r from-transparent to-neon-cyan/50"></div>
              <span className="mx-4 text-xs font-bold text-neon-cyan uppercase tracking-[0.2em]">FieldOps Platform Vision</span>
              <div className="h-[1px] w-12 bg-gradient-to-l from-transparent to-neon-cyan/50"></div>
            </div>
            <p className="text-sm md:text-base font-lexend text-white/90 mb-3 italic">
              From "FieldOps understands the document" to "FieldOps understands what the organization is doing around the document."
            </p>
            <p className="text-xs md:text-sm text-gray-400 leading-relaxed mb-2">
              FieldOps owns the context, communication, coordination, and institutional memory around project work.
            </p>
            <p className="text-[11px] md:text-xs text-gray-500 font-medium">
              FieldOps becomes the organizational layer that preserves everything the team said, decided, and did around those documents in the context of the project that produced it.
            </p>
          </div>
        </div>
      </main>
